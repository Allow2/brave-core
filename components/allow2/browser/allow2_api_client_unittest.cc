...
class Allow2ApiClientTest : public testing::Test {
 protected:
  ...
  std::string CreateSuccessfulPairResponse() {
    base::Value::Dict response;
    response.Set("success", true);
    response.Set("userId", "user123");
    response.Set("pairId", "pair456");
    response.Set("pairToken", "token789");

    base::Value::List children;
    base::Value::Dict child1;
    child1.Set("id", 1001);
    child1.Set("name", "Emma");
    child1.Set("pinHash", "sha256:abc123");
    child1.Set("pinSalt", "salt1");
    children.Append(std::move(child1));

    base::Value::Dict child2;
    child2.Set("id", 1002);
    child2.Set("name", "Jack");
    child2.Set("pinHash", "sha256:def456");
    child2.Set("pinSalt", "salt2");
...
TEST_F(Allow2ApiClientTest, PairDevice_Success) {
  bool callback_called = false;
  PairResponse captured_response;

  api_client_->PairDevice(
      "parent@example.com", "password123", "device_token_abc", "Test Device",
      base::BindOnce(
          [](bool* called, PairResponse* captured,
  ...
  test_url_loader_factory_->AddResponse(
      "https://api.allow2.com/api/pair", CreateSuccessfulPairResponse());

  task_environment_.RunUntilIdle();

  EXPECT_TRUE(callback_called);
  EXPECT_TRUE(captured_response.success);
  EXPECT_EQ("user123", captured_response.user_id);
  EXPECT_EQ("pair456", captured_response.pair_id);
  EXPECT_EQ("token789", captured_response.pair_token);
  ASSERT_EQ(2u, captured_response.children.size());
  EXPECT_EQ("Emma", captured_response.children[0].name);
  EXPECT_EQ("Jack", captured_response.children[1].name);
}

TEST_F(Allow2ApiClientTest, PairDevice_InvalidCredentials) {
  bool callback_called = false;
  PairResponse captured_response;

  api_client_->PairDevice(
      "wrong@example.com", "wrongpass", "device_token_abc", "Test Device",
      base::BindOnce(
          [](bool* called, PairResponse* captured,
             const PairResponse& response) {
            *called = true;
  ...
  EXPECT_EQ("Invalid credentials", captured_response.error);
}

TEST_F(Allow2ApiClientTest, PairDevice_NetworkError) {
  bool callback_called = false;
  PairResponse captured_response;
  ...
  test_url_loader_factory_->AddResponse(
      GURL("https://api.allow2.com/api/pair"),
...
TEST_F(Allow2ApiClientTest, PairDeviceWithCode_Success) {
  bool callback_called = false;
  PairResponse captured_response;

  api_client_->PairDeviceWithCode(
      "123456", "device_token_abc", "Test Device",
      base::BindOnce(
          [](bool* called, PairResponse* captured,
             const PairResponse& response) {
            *called = true;
  ...
  test_url_loader_factory_->AddResponse(
      "https://api.allow2.com/api/pairWithCode",
      CreateSuccessfulPairResponse());
